...
async def run_auto_stop_long_shifts_loop(
    threshold_hours: float = 15.0, interval_seconds: int = 300
) -> None:
    """
    Periodically scans for employees whose latest punch is a CLOCK_IN and whose
    shift has exceeded `threshold_hours`. If found, automatically creates a
    CLOCK_OUT entry at the threshold time and records an admin audit entry.

    - threshold_hours: Maximum allowed shift length before auto stop
    - interval_seconds: How often to run the scan (default 5 minutes)
    """
    ...
    while True:
        ...
        try:
            ...
            print(f"[SHIFT_GUARD] Error in auto-stop loop: {e}")
